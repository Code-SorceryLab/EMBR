...
`src/eval/emotion_flip.py`, `python -m eval.emotion_flip`, figure `data/figures/affective_indexing.png`.
...
These p values are now produced by the harness (`src/eval/stats.py:mcnemar_exact`, called from
...
**The mechanism is not what it looks like, and `src/eval/attribution.py` proves it.** The obvious
...
Branch `lagged-mood-congruence`, `src/eval/provenance.py`, `python -m eval.provenance`. Two
...
One reviewer opening `src/eval/baselines.py` ends the paper with that sentence.
...
**Measured on 2026-08-22, and the asymmetry collapses.** `src/eval/poignancy.py` asks
...
runs under four tag conditions against every arm (`src/eval/attacks.py::tag_variants`). Three
...
`src/eval/backends.py` puts `mnemosyne-hermes` 3.15.1 behind the harness's retrieval seam
...
1. **The demo could be broken out of by a memory's text, and it is fixed.** `src/eval/report/build_demo.py`
...
to claim every injection sits at valence 0.9, arousal 0.8, but `src/eval/attacks.py` spans |v|
...
sweeping it would measure the wrong variable. That experiment is retired. `src/eval/attribution.py`
...
1. **Lagged mood congruence.** Score congruence against the character's mood *before* this
   ...
   variant, `embr_lagged_mood`, measured by extending `src/eval/attribution.py`. No new store, no
...
`src/eval/attribution.py` can be run against it too: per-signal attribution on a third-party
