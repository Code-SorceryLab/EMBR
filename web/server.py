"""A tiny stdlib HTTP server for the web demo. No dependency, no framework, CPU only.

Serves the single page and its assets, and a small JSON API backed by one `GameSession`. The
session runs on the stub, so the demo never needs a model or the network. One session per
server process is enough for a local demo; state lives in memory and resets on request.
...
from web.game import GameSession
...
class DemoHandler(BaseHTTPRequestHandler):
    ...
    # One session shared by the process, set by `serve` before the server starts.
    session: GameSession
...
def build_server(port: int = 8000) -> ThreadingHTTPServer:
...
def serve(port: int = 8000, open_browser: bool = True) -> None:
    ...
    print(f"EMBR web demo on {url}  (Ctrl+C to stop). Runs on the stub, no model needed.")
    if open_browser:
        webbrowser.open(url)
    try:
